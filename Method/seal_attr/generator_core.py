...
class Agent(nn.Module):
    ...
    def forward(self, x_attrs, x_seeds, x_nodes, indptr):
        ...
        node_scores = self.node_score_layer(h).squeeze(1)  # [*]  1,d
        batch = []
        for startpoint, endpoint, candidate_endpoint in indptr:
            if startpoint == endpoint:
                raise ValueError('Finished Episode!')
            else:
                trajectories = self.pooling_layer(h[candidate_endpoint: endpoint])
                boundary = self.pooling_layer(h[startpoint:candidate_endpoint])
                global_z = torch.div(trajectories, (trajectories + boundary))

                value = self.value_layer(global_z).squeeze()  
                node_logits = torch.log_softmax(node_scores[startpoint:candidate_endpoint], 0)  # [r-l]
                stopping_logits = torch.log_softmax(self.stopping_score_layer(global_z), 1).squeeze(0)  # [2] tensor([-0.6931, -0.6931], device='cuda:0')
