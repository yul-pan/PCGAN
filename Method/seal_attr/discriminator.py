...
class Discriminator:
    ...
    def predict_step(self, pred_comms, model_pth):  
        subgs = []
        for nodes in itertools.chain(pred_comms):
            subg = self.prepare_graph(nodes)
            subgs.append(subg)
        if self.with_attr:
            batch_graph = dgl.BatchedDGLGraph(subgs, ['state', 'feats'], None)
            ...
            batch_graph = dgl.BatchedDGLGraph(subgs, 'state', None)
        labels = torch.ones(len(pred_comms)).long().to(self.device)
        data = torch.load(model_pth)
        self.model.load_state_dict(data['d'])
        self.model.eval()
        logits = self.model(batch_graph)
        out = F.softmax(logits, dim=1)
        max_value,index = t.max(out,1)
        #loss = F.nll_loss(logits, labels)
        #acc = (torch.argmax(logits, 1) == labels).float().mean().item()
